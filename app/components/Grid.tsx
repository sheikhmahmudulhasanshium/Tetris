import React, { useState, useEffect } from 'react';

interface GridProps {
    onScoreUpdate: (score: number) => void;
    onLevelUpdate: (level: number) => void;
    onLinesUpdate: (lines: number) => void;
    onNextShapeUpdate: (nextShape: number[]) => void;
    score: number;     // Define score prop
    level: number;     // Define level prop
    lines: number;     // Define lines prop
    isPaused: boolean; // Define isPaused prop
}

const Grid: React.FC<GridProps> = ({
    onScoreUpdate,
    onLevelUpdate,
    onLinesUpdate,
    onNextShapeUpdate,
    score: propScore,
    level: propLevel,
    lines: propLines,
    isPaused,
}) => {
    const rows = 22;
    const columns = 12;

    // Game state
    const [grid, setGrid] = useState<boolean[]>(Array(rows * columns).fill(false));
    const [currentShape, setCurrentShape] = useState<number[]>([]);
    const [shapeIndex, setShapeIndex] = useState(0);
    const [nextShapeIndex, setNextShapeIndex] = useState(1);
    const [position, setPosition] = useState(columns * 2 + 4);

    // Shapes configuration
    const shapesList = [
        [1, 2, columns + 1, columns + 2],           // squareShape
        [1, columns + 1, columns * 2 + 1, columns * 3 + 1], // lShape
        [2, columns + 2, columns * 2 + 2, columns * 2 + 1], // jShape
        [1, 2, columns + 2, columns + 3],           // zShape
        [2, 3, columns + 1, columns + 2],           // sShape
        [1, columns + 1, columns + 2, columns + 3], // tShape
        [1, 2, columns + 2, columns + 3],           // reverseZShape
    ];

    // Utility functions
    const placeShape = (shape: number[], pos: number) => shape.map(index => index + pos);

    const checkCollision = (shape: number[], offset: number) =>
        shape.some(index => {
            const pos = index + offset;
            const x = pos % columns;
            const y = Math.floor(pos / columns);
            return x < 1 || x >= columns - 1 || y >= rows - 1 || (y > 0 && grid[pos]);
        });

    const mergeShapeToGrid = (shape: number[]) => {
        setGrid(prevGrid =>
            prevGrid.map((cell, index) => (shape.includes(index) || cell ? true : false))
        );
    };

    const clearFullLines = () => {
        setGrid(prevGrid => {
            let newGrid = [...prevGrid];
            let linesCleared = 0;

            for (let row = rows - 2; row >= 0; row--) {
                if (newGrid.slice(row * columns + 1, row * columns + columns - 1).every(Boolean)) {
                    for (let col = 1; col < columns - 1; col++) {
                        newGrid[row * columns + col] = false;
                    }
                    linesCleared++;
                }
            }

            if (linesCleared > 0) {
                onLinesUpdate(propLines + linesCleared);
                onScoreUpdate(propScore + linesCleared);
                if (propLines + linesCleared >= propLevel * 10) {
                    onLevelUpdate(propLevel + 1);
                }
            }

            // Apply gravity effect
            for (let row = rows - 3; row >= 0; row--) {
                for (let col = 1; col < columns - 1; col++) {
                    if (newGrid[row * columns + col] && !newGrid[(row + 1) * columns + col]) {
                        newGrid[(row + 1) * columns + col] = true;
                        newGrid[row * columns + col] = false;
                    }
                }
            }

            return newGrid;
        });
    };

    const handleMove = (offset: number) => {
        const newShape = currentShape.map(index => index + offset);
        const isValidMove = !checkCollision(newShape, 0) && newShape.every(index => index >= 0 && index < rows * columns);

        if (isValidMove) {
            setCurrentShape(newShape);
            setPosition(prev => prev + offset);
        } else if (offset === columns) {
            mergeShapeToGrid(currentShape);
            clearFullLines();
            setShapeIndex(nextShapeIndex);
            setNextShapeIndex((nextShapeIndex + 1) % shapesList.length);
            setPosition(columns * 2 + 4);
            setCurrentShape(placeShape(shapesList[nextShapeIndex], columns * 2 + 4));
            onNextShapeUpdate(shapesList[(nextShapeIndex + 1) % shapesList.length]);
        }
    };

    const rotateShape = () => {
        const centerIndex = currentShape[0];
        const centerX = centerIndex % columns;
        const centerY = Math.floor(centerIndex / columns);

        const newShape = currentShape.map(index => {
            const x = index % columns;
            const y = Math.floor(index / columns);
            const newX = centerX + centerY - y;
            const newY = centerY - centerX + x;
            return newY * columns + newX;
        });

        const isValidRotation = !checkCollision(newShape, 0) && newShape.every(index => index >= 0 && index < rows * columns);

        if (isValidRotation) {
            setCurrentShape(newShape);
        }
    };

    // Effect to handle game logic and gravity
    useEffect(() => {
        if (!isPaused) {
            const interval = setInterval(() => {
                handleMove(columns);
            }, 1000 - (propLevel - 1) * 100);
            return () => clearInterval(interval);
        }
    }, [currentShape, isPaused, propLevel]);

    // Initialize the first shape on component mount
    useEffect(() => {
        setCurrentShape(placeShape(shapesList[shapeIndex], columns * 2 + 4));
        onNextShapeUpdate(shapesList[nextShapeIndex]);
    }, []);

    // Handle keyboard events for controls
    const handleKeyDown = (event: KeyboardEvent) => {
        switch (event.key) {
            case 'ArrowLeft':
                handleMove(-1);
                break;
            case 'ArrowRight':
                handleMove(1);
                break;
            case 'ArrowDown':
                handleMove(columns);
                break;
            case 'ArrowUp':
                rotateShape();
                break;
            case ' ':
                event.preventDefault();
                handlePause();
                break;
            default:
                break;
        }
    };

    // Effect to add event listener for keyboard controls
    useEffect(() => {
        window.addEventListener('keydown', handleKeyDown);
        return () => {
            window.removeEventListener('keydown', handleKeyDown);
        };
    }, [handleKeyDown]);

    // Toggle pause state
    const handlePause = () => {
        // Implement pause logic here if needed
    };

    // Render grid items based on current game state
    const gridItems = [];
    for (let i = 0; i < rows * columns; i++) {
        const isShape = currentShape.includes(i);
        const isEdge = i % columns === 0 || i % columns === columns - 1 || i < columns || i >= (rows - 1) * columns;
        gridItems.push(
            <div
                key={i}
                className={`col-span-1 border ${grid[i] ? 'bg-slate-500' : isShape ? 'bg-slate-300' : isEdge ? 'bg-cyan-950' : ''}`}
                style={{ height: '20px', width: '20px' }}
            >
                &nbsp;
            </div>
        );
    }

    // Return the rendered grid and controls
    return (
        <div className="flex flex-col items-center relative">
            <div className="flex justify-center items-center border border-black my-4">
                <div className="grid grid-cols-12 gap-1 justify-center items-center">
                    {gridItems}
                </div>
            </div>
            <div className="flex justify-center text-center absolute right-0">
                <button
                    onClick={handlePause}
                    className="bg-blue-500 text-white rounded flex items-center justify-center ml-4"
                >
                    {isPaused ? 'Play' : 'Pause'}
                </button>
            </div>
        </div>
    );
};

export default Grid;
